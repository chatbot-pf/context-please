#!/bin/bash
# Script to run evaluation with Milvus vector database
...
echo -e "${BLUE}============================================${NC}"
echo -e "${BLUE}  Milvus Evaluation Runner${NC}"
echo -e "${BLUE}============================================${NC}"
...
if [ -z "$MILVUS_ADDRESS" ]; then
    echo -e "${YELLOW}Warning: MILVUS_ADDRESS is not set${NC}"
    echo -e "${YELLOW}Using default: localhost:19530${NC}"
...
OUTPUT_DIR="${OUTPUT_DIR:-retrieval_results_milvus}"
...
echo -e "\n${BLUE}Step 3: Running evaluation with Milvus...${NC}"
...
echo ""
echo "2. To compare with Qdrant, run:"
